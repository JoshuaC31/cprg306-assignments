        ...
        <div className="justify-items-center w-1/2 p-4 m-3">
            ...
            <div>
                <ul>
                    {meals.length > 0 ? (
                        meals.map((meal) => (
                            <div className="drop-shadow-md bg-white border-4 border-red-600 rounded-lg p-3 grid grid-cols-2 m-3" key={meal.idMeal}>
                                <img className="w-24 h-24"src={meal.strMealThumb} alt={meal.strMeal}/>
                                <h3 className="text-black font-mono m-2">{meal.strMeal}</h3>
                            </div>
                        ))
                    ) : (
